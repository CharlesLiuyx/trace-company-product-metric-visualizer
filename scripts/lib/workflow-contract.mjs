...
export const WORKFLOW_PROTOCOLS = Object.freeze({ sourceFacts: SOURCE_FACTS_PROTOCOL, textSourceClassification: 'source-classification/v2', metricSourceCoverage: METRIC_COVERAGE_PROTOCOL, metricObservations: METRIC_RECORD_PROTOCOL, artifactManifest: ARTIFACT_MANIFEST_PROTOCOL, checkpoint: CHECKPOINT_PROTOCOL, publication: PUBLICATION_PROTOCOL, releaseAttempt: 'release-attempt/v1' });
export async function workflowCommandReference(root = rootDir) {
  const pkg = JSON.parse(await readFile(path.join(root, 'package.json'), 'utf8'));
  const scripts = Object.entries(pkg.scripts).filter(([key]) => /^(record:workflow|publish:datasets|release:dataset|view:published|(?:verify|update):(?:metric-catalog|metrics|asset-catalog|feedback-patterns|workflow|workflow-reference|workflow-graph))$/.test(key));
