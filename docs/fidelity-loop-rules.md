...
8. 检查多入口/多出口节点的连接线 socket 顺序，尤其是终端利润节点、税费分流、other income/expense 汇入和费用明细拆分。
9. 检查关联 label-node 外框关系，尤其是上下排列的 label 与柱子中心线和 5px 间距。
10. 检查 `annotationsSvg` 中 KPI/stat card、图例框、徽章框等注释容器的内部对齐：容器外框和内部文字/图标组外框必须按参考图意图居中、对齐或留出明确边距。
11. 如果数据集新增或修改了非默认语言覆盖，渲染非默认语言并检查本地化后的固定布局文本、缩写、标点和画布边界。
12. 修改数据、布局或渲染器。
13. 进入细微调参阶段时，并行尝试多个候选值或小范围参数组合，统一计分后只保留整体效果最好的结果。
14. 重复以上步骤，直到必须死磕的问题全部消除，且整体 Diff 改善进入平台期。
15. 清理临时产物。
...
每一轮 Loop 都必须检查所有有多条 `sourceLinks` 或 `targetLinks` 的节点。检查口径以渲染后的 SVG path 和节点外框为准，不以源码中的 link 数组位置、节点 `order`、或上一轮的视觉印象为准。
...
6. 对短辅助节点，如果原图显示节点贴近 label，必须放大该局部区域检查原图、候选图和 Diff 图，并记录节点 bbox、label bbox、`centerDelta` 和 `edgeGap`；不能只接受全图指标或主连接线形态。
7. 只有原图明确使用了不同间距、或为避免文本越界/交叠必须偏离时，才可以破例；破例必须在本轮记录中写明原因。

`pnpm verify:d3 -- <dataset-key>` 每轮会输出 label-node 竖向组合、相邻 label 间距、以及左右相邻 label-node 横向关系的审计项。左右相邻 label 与 node 发生横向交叠时，验证脚本必须失败；其他审计输出用于提醒和定位，最终是否接受仍必须结合原图、候选图和 Diff 图目视确认。
...
每一轮 Loop 都必须检查有明确背景容器的注释组。检查口径以渲染后的 SVG 外框为准：
...
- 如果保留手写 baseline，至少在最后一轮用浏览器 `getBBox()` 或等价方法记录容器外框、内部 union 外框、中心差和四边边距。
...
5. 可接受残留已经记录原因。
6. 有背景容器的固定注释已经通过内部对齐检查，没有文字/图标组贴边、偏心或被圆角区域挤压。
7. 左右相邻 label-node 没有横向交叠；如果边界间距小于 `5px`，已经有参考图依据或避让原因。
8. 新增或修改过的非默认语言覆盖已经通过本地化布局检查，固定布局文本没有无意 fallback、混排、缩写损坏、重叠或越界。

全图相似度高不代表可以结束。只要存在接口错位、文本交叠、文本位置错误、关联 label-node 外框对齐/间距明显不合理、注释容器内部内容偏心/贴边，或文本越界，就必须继续修。
