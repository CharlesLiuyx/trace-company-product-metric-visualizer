...
当一个 `data(batch)` 提交或同一批交付包含多个独立对象（例如多个数据集、
Source 图片或产品条目）时，提交正文必须记录本批次的人工审阅介入情况。
统计单位是“对象”，不是修改意见条数；同一对象即使收到多条人工修改意见，
也只计为一个“有人工介入”对象。

- `人工零介入率` = 人工审阅中未提出任何修改意见的对象数 ÷ 批次对象总数；
- `人工介入率` = 人工审阅中提出过至少一项修改意见的对象数 ÷ 批次对象总数；
- 两个分子之和必须等于批次对象总数，两个比率互为补数；
- 同时保留原始分数和百分比，百分比四舍五入到一位小数；
- 只记录已经发生的人工审阅结果。状态未知或尚未审阅的对象不得推测为
  “零介入”，应先完成统计再提交。
...
- Intervention items: 5
- Human zero-intervention rate: 10/15 = 66.7%
- Human intervention rate: 5/15 = 33.3%
```

例如 15 个数据集中有 10 个未被人工提出修改意见，则人工零介入率是
`10/15 = 66.7%`，人工介入率是 `5/15 = 33.3%`。不要把前者简称为
“人工介入率”，以免把指标方向写反。
