...
| CB-003 | 可见短柱被缩成不可见锚点或亚像素柱；同批曾对最小柱高各自取值——Visa 6px、SAP/Comcast 3px（0d7cc54、72644e5）。Lyft Q3 FY25 的 Other 72×1px 浅绿柱、Block Q3 FY25 的 Amortization 71×1px 浅红柱、Marriott Q3 FY25 的 D&A 71×1px 浅红末端柱、Paramount Q3 FY25 的 Eliminations 71×2px 浅红柱，以及 Axon Q3 FY25 的 Operating loss 72×2px 红柱均曾被误分类为隐藏锚点；Marriott 首次修复又错误放大为 3px，用户要求严格恢复 1px（2026-07-16）。 | 短辅助柱（interest、tax、other、amortization、D&A、eliminations、operating loss…） | ambiguous-rule → execution-gap recurrence | T13 + T21 `node-face-policy/v1` 硬门；例外必须由 Source Coverage 绑定原生 face（B15/T14）；T22 带值 Other 强制可见柱面，hidden-anchor 分类非法；Block/Marriott/Paramount/Axon 增加数据集契约测试，固定各自 Source face、非透明颜色及原生 link endpoint；用户明确要求原生高度时不得用共享可见高度替代 |
...
| CB-007 | 隐藏锚点误判：该隐形的画出可见痕、该可见的做成透明锚点、引导线丢失（d519811、636dea5）；Texas Instruments Q4 FY25 的 Other $40M 微流段曾被做成无色锚点；Block Q3 FY25 的 Amortization、Marriott Q3 FY25 的 D&A 71×1px 连续浅红柱、Paramount Q3 FY25 的 Eliminations 71×2px 浅红柱与 Axon Q3 FY25 的 Operating loss 72×2px 红柱也被误判为透明锚点，均由用户截图指出 | 细流带+引导线、无柱面对象 | execution-gap recurrence | B7 + T12 结构化 native-pixel 证据与独立确认；T12a 注释入口；可见短柱改走 B15/T13/T14/T21；带值 Other 由 T22 直接禁止 hidden-anchor；Block/Marriott/Paramount/Axon 数据集契约测试作为 hard-gate 固定 Source face，且同批排查确认其他新 Adapter 不含非零透明节点，仅无 Source 对象的技术 bridge 可保持 hidden anchor |
