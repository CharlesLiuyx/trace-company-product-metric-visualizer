...
| CB-004 | 固定布局 label 组整体偏离参考位置，已收敛数据集因此返工（alphabet/amazon/amd 批次；41c2f20）；Amgen Q1 FY26 的 Operating expenses label 又因参考中心误测而比柱体中心轴右偏约 31px，用户要求左移并居中；Domino's Q1 FY26 的 Supply chain 金额/同比块随后被指出离柱体过远，用户要求下移并固定为柱上方 5px（2026-07-22） | 每个 `layout.labels.*` 组 | execution-gap recurrence | T18 `labelPositionAudit` + Plan 编译强制 `measured-label-position`；Amgen 数据集契约测试额外固定英中 label x 与柱体中心轴差不超过 0.5px；Domino's 数据集契约测试固定英中金额块 `top`，并由逐 locale `labelLayoutAudit` 量化确认柱上间距为 5px。同批 Align、Teladoc、P&G、Booking、Chipotle、GE Vernova、Tilray 的最新逐 locale 证据已横向排查，T18 与 label-layout violations 均为 0 |
