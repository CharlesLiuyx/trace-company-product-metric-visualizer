...
| CB-029 | Affirm Q1 FY26 的 Gain on sale of loans 名称块位于柱中心上方；Airbnb Q3 FY25 又出现四个区域名称未与柱面居中、LATAM/APAC 金额块未贴近短柱的同型反馈 | 侧置名称块与金额/备注分块布局，名称块应以 node face 为中心而完整 label union 仍保留源图关系 | execution-gap | 为对象声明 `centered-side-label`，并按 T18 保留 Source `referenceBBox`、记录用户批准的 `approvedTargetBBox`；渲染审计现在也会从“独立金额同轴块 + 侧置名称块”自动推导 B3/T7 hard gate，每个 locale 强制中心差 `<=4px` | 若金额语义无法由渲染文本稳定识别，则为 label block 增加显式 role，并让同一推导门槛改用 role 而非文本模式 |
