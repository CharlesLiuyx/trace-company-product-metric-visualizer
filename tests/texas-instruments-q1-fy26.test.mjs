...
function dataset() {
  const context = loadClassicScripts(['data/datasets/texas-instruments-q1-fy26.js']);
  return context.DATASETS.find((entry) => entry.key === 'texas-instruments-q1-fy26');
}

test('TI Q1 FY26 keeps Source-visible Other and restructuring faces', () => {
  const data = dataset();
  const nodes = new Map(data.nodes.map((node) => [node.id, node]));
  const links = data.links.map((link) => `${link.source}->${link.target}`);

  assert.equal(data.layout.nodes.other_income.height, 4);
  assert.equal(data.layout.nodes.restructuring.height, 2);
  assert.equal(nodes.get('other_income').color, '#2ca02c');
  assert.equal(nodes.get('restructuring').color, '#d40000');
  assert.ok(links.includes('other_income->net_profit'));
  assert.ok(links.includes('operating_expenses->restructuring'));
});

test('TI Q1 FY26 preserves Source endpoint order', () => {
  const data = dataset();
  ...
  assert.deepEqual(Array.from(operatingExpensesOut), ['rnd', 'sga', 'restructuring']);
