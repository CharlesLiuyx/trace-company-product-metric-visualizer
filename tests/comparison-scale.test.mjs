...
test('all 947 registered View Adapters calibrate to one view-units/USD scale with the known fallback set', () => {
  ...
  assert.equal(datasets.length, 947);
  assert.equal(financialByKey.size, 947);

  const plan = context.TraceComparisonScale.createPlan(entries);

  assert.equal(plan.status, 'calibrated');
  assert.equal(plan.diagnostics.length, 0);
  assert.equal(plan.measurements.length, 947);
  assert.deepEqual(
    Array.from(plan.measurements, (measurement) => measurement.key),
    inputKeys,
